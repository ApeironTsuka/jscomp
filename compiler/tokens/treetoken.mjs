...
export class TreeToken extends Token {
  constructor(type, label, value) { super(type, label, value); this.index = -1; this.children = []; }
  /*
  ...
  */
  ...
  static copyOfWithChildren(token, virt, copyOrig) {
    let t = new TreeToken(token.type, token.label, token.value);
    t.virt = token.virt;
    t.orig = copyOrig ? token.orig : token;
    t.index = token.index;
    for (let i = 0, { children } = token, l = children.length; i < l; i++) { t.children.push(TreeToken.copyOfWithChildren(children[i], children[i].virt, copyOrig)); }
    return t;
  }
}
